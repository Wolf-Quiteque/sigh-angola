...
Paciente, marcação, episódio, triagem, consulta, pedido de exame e internamento são entidades distintas ligadas por identificadores. Enfermarias, camas, artigos, lotes e fornecedores são dados de referência da unidade. A chegada cria exactamente um episódio; uma triagem avança esse episódio para a fila médica; uma consulta contém prescrições e, ao terminar, encerra o episódio. Registos finalizados recebem adendas imutáveis em vez de edição directa. Um pedido de exame nasce numa consulta em curso e guarda paciente, episódio e consulta; percorre os estados da sua via — laboratório ou imagiologia — e só depois da validação médica entra no processo clínico como resultado definitivo. Um internamento nasce de uma consulta com destino «Internamento», ocupa exactamente uma cama e liberta-a na transferência ou na alta, sempre na mesma operação que altera o internamento. O stock de um artigo é sempre a soma dos seus lotes; nenhuma saída o deixa negativo e cada movimento guarda o saldo resultante. A dispensação liga um lote a um item de prescrição e a um paciente. Histórico e contadores são calculados destas mesmas entidades.

Fases 1 a 5 suportam dados locais após carregamento da aplicação e recarregamento com o servidor Next disponível. Não inclui service worker, instalação PWA, sincronização, sessões seguras ou utilização de dados clínicos reais. Perfis são uma simulação de UX. O conjunto JSON e exportações são inteiramente fictícios. O reset afecta somente a chave da demo neste navegador. Ao abrir dados de uma versão anterior, o repositório aplica as migrações em cadeia até à versão 5, acrescentando as novas colecções e preservando pacientes, marcações, episódios, registos clínicos, auditoria e revisão.
